...
echo "[0/6] Testing SSH connectivity to Machine B..."
...
echo "[1/6] Creating directory structure on Machine B..."
$SSH_CMD "mkdir -p \
...
# --- [2] Install system packages and Python dependencies ---
echo "[2/6] Installing system dependencies on Machine B..."
$SSH_CMD "sudo apt-get update -qq"
$SSH_CMD "sudo apt-get install -y iperf3 powerstat powertop python3 python3-pip"
# pip install with fallback for Ubuntu 23+ externally-managed environments
$SSH_CMD "pip3 install --quiet psutil pandas 2>/dev/null || \
          pip3 install --break-system-packages --quiet psutil pandas"
echo "  System dependencies installed."

# --- [3] Install Scaphandre ---
if [[ "$RAPL" == "true" ]]; then
    echo "[3/6] Installing Scaphandre on Machine B (RAPL available)..."
    ...
    if [[ -n "$SCAPH_LOCAL" ]]; then
        ...
        echo "  Downloading scaphandre v1.0.0 from GitHub releases..."
    ...
    echo "[3/6] Skipping Scaphandre (RAPL not available — PowerStat is primary tool)."
fi

# --- [4] Configure passwordless sudo for measurement tools ---
echo "[4/6] Configuring passwordless sudo for measurement tools..."
...
# --- [5] Copy scripts to Machine B ---
echo "[5/6] Copying scripts to Machine B..."
...
echo "[6/6] Verifying installation on Machine B..."

echo -n "  iperf3:      "
$SSH_CMD "iperf3 --version 2>&1 | head -1"

echo -n "  powerstat:   "
$SSH_CMD "powerstat -h 2>&1 | head -1 || echo OK"
...
echo -n "  pandas:      "
$SSH_CMD "python3 -c 'import pandas; print(\"OK\")'"

echo -n "  client_daemon: "
$SSH_CMD "test -x ~/edge_cloud_study/client_daemon.sh && echo OK || echo MISSING"

echo -n "  parse_scaphandre: "
$SSH_CMD "test -f ~/edge_cloud_study/parse_scaphandre.py && echo OK || echo MISSING"

echo -n "  parse_powerstat:  "
$SSH_CMD "test -f ~/edge_cloud_study/parse_powerstat.py && echo OK || echo MISSING"

# --- Update AGENT_MEMORY.json ---
echo "Updating AGENT_MEMORY.json..."
python3 - << PYEOF
import json
...
print("  AGENT_MEMORY.json updated: client_machine_ready=true, client_machine_tools_installed=true")
PYEOF
